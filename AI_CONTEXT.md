...
- **Theme-token layer.** The facelift swaps visuals through a single theme object that render helpers read. It must change no state, validation, scoring, market, or turn logic, and no `registerTarget` wiring.
